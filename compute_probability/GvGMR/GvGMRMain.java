...
public class GvGMRMain
{
	public static void main(String[] args) throws Exception
	{
		Job run1 = Job.getInstance();
		run1.setJarByClass(GvGMRMain.class);
		run1.setJobName("GvG Map Reduce");
		
		FileInputFormat.addInputPath(run1, new Path(args[0]));
		FileOutputFormat.setOutputPath(run1, new Path(args[1]));

		run1.setMapperClass(GvGMapper.class);
		run1.setReducerClass(GvGReducer.class);
